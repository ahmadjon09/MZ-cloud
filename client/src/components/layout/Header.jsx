    ...
    <header className="h-16 px-4 sm:px-6 flex items-center justify-between border-b border-white/10 bg-[#1e2329]/90 backdrop-blur-md shadow-lg transition-colors duration-200 z-20 font-sans">
      ...
      <div className="flex items-center space-x-3 overflow-hidden">
        ...
        <button
          ...
          className="p-2 sm:hidden text-slate-300 hover:text-[#2481cc] rounded-lg transition-colors"
        ...
        <div className="flex items-center space-x-2 text-white font-semibold text-lg">
          <FiCloud className="w-6 h-6 text-[#2481cc] flex-shrink-0" />
          <span className="truncate hidden xs:inline">{t('app.title')}</span>
        </div>

        {/* Dynamic Folder Breadcrumbs */}
        {breadcrumbs.length > 0 && (
          <div className="flex items-center space-x-1 text-sm text-slate-400 overflow-hidden">
            <FiChevronRight className="w-4 h-4 text-slate-500 flex-shrink-0" />
            <button
              onClick={() => setActiveFolderId(null)}
              className="hover:text-[#2481cc] truncate"
            >
              Root
            </button>
            {breadcrumbs.map((crumb) => (
              <React.Fragment key={crumb.id}>
                ...
                <button
                  ...
                  className="flex items-center space-x-1 hover:text-[#2481cc] truncate"
                >
      ...
      <div className="flex-1 max-w-md mx-4 hidden md:block">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-2 text-sm text-slate-400 bg-[#17212b]/80 rounded-full border border-white/10 hover:border-[#2481cc] transition-colors duration-150"
        >
          <div className="flex items-center space-x-2">
            <FiSearch className="w-4 h-4 text-[#2481cc]" />
            <span>{t('app.searchPlaceholder')}</span>
          </div>
          <kbd className="px-1.5 py-0.5 text-xs font-semibold text-slate-400 bg-black/40 rounded">
            Cmd K
          </kbd>
        </button>
      </div>
      ...
      <div className="flex items-center space-x-2 sm:space-x-3">
        ...
        <button
          onClick={() => setSearchModalOpen(true)}
          className="p-2 md:hidden text-slate-300 hover:text-[#2481cc] rounded-full transition-colors"
          title="Search"
        >
        ...
        <button
          ...
          className="p-2 text-slate-300 hover:text-[#2481cc] hover:bg-white/5 rounded-full transition-colors"
        >
        ...
        <div className="relative group">
          <button
            className="flex items-center space-x-1 p-2 text-xs font-semibold text-slate-300 hover:text-[#2481cc] hover:bg-white/5 rounded-full transition-colors uppercase"
            title="Switch Language"
          >
            <FiGlobe className="w-4 h-4" />
            <span className="hidden sm:inline">{i18n.language || 'EN'}</span>
          </button>
          <div className="absolute right-0 mt-1 w-28 py-1 bg-[#1e2329] rounded-lg shadow-lg border border-white/10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 z-30">
            <button
              onClick={() => changeLanguage('en')}
              className="w-full text-left px-3 py-1.5 text-xs hover:bg-[#2481cc] hover:text-white transition-colors"
            >
              🇺🇸 English
            </button>
            <button
          ...
          <div className="flex items-center space-x-2.5 pl-3 border-l border-white/10">
            ...
            <div className="hidden md:block text-xs">
              <div className="font-semibold text-white flex items-center space-x-1">
                ...
                {user.isPremium && <FiStar className="w-3 h-3 fill-amber-500 text-amber-500" title="Telegram Premium" />}
              </div>
