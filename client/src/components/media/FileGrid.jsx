/**
 * Saved Messages / Cloud Storage Media Grid & List View (react-icons/fi Only)
...
export default function FileGrid({ files = [], total = 0, isLoading, activeCategory, isTrash = false }) {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className="relative flex-1 flex flex-col p-4 sm:p-6 min-h-screen font-sans"
    >
      {/* Drag & Drop Overlay */}
      {isDragging && (
        <div className="absolute inset-4 z-40 bg-[#2481cc]/90 backdrop-blur-md rounded-3xl border-2 border-dashed border-white flex flex-col items-center justify-center text-white p-8 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6 bg-[#1e2329]/90 backdrop-blur-md p-4 rounded-2xl border border-white/10 shadow-sm">
        ...
        <div className="flex items-center space-x-4 w-full sm:w-auto justify-between sm:justify-end">
          ...
          <div className="flex items-center space-x-2 text-xs">
            <select
              ...
              className="bg-[#17212b] border border-white/10 rounded-lg px-2.5 py-1.5 text-slate-200 focus:outline-none focus:border-[#2481cc]"
            >
          ...
          <div className="flex items-center bg-[#17212b] p-1 rounded-xl border border-white/10">
            <button
              onClick={() => setViewMode('grid')}
              title={t('actions.grid')}
              className={`p-1.5 rounded-lg transition-colors ${
                viewMode === 'grid' ? 'bg-[#2481cc] text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${
                viewMode === 'masonry' ? 'bg-[#2481cc] text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${
                viewMode === 'list' ? 'bg-[#2481cc] text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
      ...
      {!isLoading && files.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center py-16 text-center">
          <div className="w-20 h-20 rounded-full bg-[#2481cc]/10 text-[#2481cc] flex items-center justify-center mb-4">
      ...
      {!isLoading && files.length > 0 && viewMode === 'list' && (
        <div className="bg-[#1e2329]/90 border border-white/10 rounded-2xl overflow-x-auto shadow-sm backdrop-blur-md">
          <table className="w-full text-left border-collapse text-xs min-w-[500px]">
            <thead>
              <tr className="border-b border-white/10 text-slate-400 bg-[#17212b]/80">
              ...
              {files.map((file) => (
                ...
                >
                  <td className="py-3 px-4 font-medium text-white flex items-center space-x-2">
                    <FiFolder className="w-4 h-4 text-[#2481cc] flex-shrink-0" />
                    <span className="truncate max-w-xs">{file.fileName}</span>
                  </td>
                  <td className="py-3 px-4 text-slate-400">
                    <span className="px-2 py-0.5 rounded bg-[#2481cc]/10 text-[#2481cc] font-semibold">
                      {file.category}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-slate-400">{formatSize(file.fileSize)}</td>
                  <td className="py-3 px-4 text-slate-400">
                    {file.folder ? (
                      <span className="flex items-center space-x-1">
                        <FiFolder className="w-3.5 h-3.5 text-[#2481cc]" />
